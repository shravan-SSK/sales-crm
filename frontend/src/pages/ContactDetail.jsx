...
import {
  ArrowLeft, Edit2, Save, X, Linkedin, Clock, CheckCircle, XCircle,
  Briefcase, GraduationCap, MapPin, Phone, Mail, Building2, ExternalLink,
  CheckCircle2,
} from 'lucide-react'
...
function LinkedInDataCard({ data }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {data.scraped_at && (
        <p className="text-xs text-gray-400">Scanned {new Date(data.scraped_at).toLocaleDateString()}</p>
      )}
    </div>
  )
}
            ...
            {linkedinData ? (
              ...
              <p className="text-xs text-gray-400">
                {scanStatus === 'pending'
                  ...
                  : 'No LinkedIn data yet. Scan to pull profile intel.'}
              </p>
            )}
            {!showLinkedinInput ? (
              <button
            ...
            {(!contact.activities || contact.activities.length === 0) ? (
              ...
              <div className="space-y-2">
                {contact.activities.map(act => {
                  ...
                  return (
                    <div key={act.id}
                      className={`flex items-start gap-3 p-3 border rounded-lg ${isOverdue ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white'}`}>
                      <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium flex-shrink-0 mt-0.5 ${colorClass}`}>
                        {act.type}
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium ${act.completed ? 'line-through text-gray-400' : 'text-gray-800'}`}>
                          {act.subject}
                        </p>
                        {act.due_date && (
