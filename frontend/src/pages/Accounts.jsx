...
import { Plus, Search, Building2, Trash2, ChevronRight, Globe } from 'lucide-react'
...
export default function Accounts() {
  ...
  const createMut = useMutation({ mutationFn: accountsApi.create, onSuccess: () => { qc.invalidateQueries(['accounts']); qc.invalidateQueries(['dashboard']); setShowModal(false) } })
  const updateMut = useMutation({ mutationFn: ({ id, data }) => accountsApi.update(id, data), onSuccess: () => { qc.invalidateQueries(['accounts']); setEditing(null) } })
  const deleteMut = useMutation({ mutationFn: accountsApi.delete, onSuccess: () => { qc.invalidateQueries(['accounts']); qc.invalidateQueries(['dashboard']) } })
              ...
              <tr key={acc.id} className="hover:bg-gray-50">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {acc.website && (
                        <a href={acc.website} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 flex items-center gap-1" onClick={e => e.stopPropagation()}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1 justify-end">
                    <button onClick={() => setEditing(acc)} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded">
                      <ChevronRight size={14} />
                    </button>
                    <button onClick={() => { if(window.confirm('Delete this account?')) deleteMut.mutate(acc.id) }}
