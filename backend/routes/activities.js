...
router.get('/', async (req, res) => {
  try {
    ...
    let query = supabase.from('activities').select('*, deals(name), contacts(first_name,last_name), leads(first_name,last_name)').order('due_date', { ascending: true });
...
router.post('/', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('activities').insert({ id: uuidv4(), type, subject, description: description||null, deal_id: deal_id||null, contact_id: contact_id||null, lead_id: lead_id||null, due_date: due_date||null, completed: 0 }).select().single();
    if (error) throw error;
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const updates = { type: type ?? existing.type, subject: subject ?? existing.subject, description: description ?? existing.description, deal_id: deal_id !== undefined ? deal_id : existing.deal_id, contact_id: contact_id !== undefined ? contact_id : existing.contact_id, lead_id: lead_id !== undefined ? lead_id : existing.lead_id, due_date: due_date !== undefined ? due_date : existing.due_date, updated_at: new Date().toISOString() };
    if (completed !== undefined) { updates.completed = completed ? 1 : 0; updates.completed_at = completed ? new Date().toISOString() : null; }
